...
import { 
  ...
  Smartphone, Instagram, ArrowRight, XCircle, PlayCircle, Trash2, Shield,
...
const Badge = ({ children, type = 'default' }) => {
  ...
  return (
    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wider flex items-center gap-1 w-max ${types[type] || types.default}`}>
      {children}
    </span>
  );
};
...
const firebaseConfig = typeof __firebase_config !== 'undefined' ? JSON.parse(__firebase_config) : {
  apiKey: "REPLACE_ME", authDomain: "REPLACE_ME", projectId: "REPLACE_ME",
  storageBucket: "REPLACE_ME", messagingSenderId: "REPLACE_ME", appId: "REPLACE_ME"
};
const app = initializeApp(firebaseConfig);
const auth = getAuth(app);
const db = getFirestore(app);
const appId = typeof __app_id !== 'undefined' ? __app_id : 'digital-infusive-agency-id';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubUsers = onSnapshot(usersRef, (snap) => {
      if (snap.exists()) setAppUsers(snap.data());
  ...
  useEffect(() => {
    ...
    return () => unsubProject();
  }, [user, activeClientId, isClientsLoaded, teamMemberProfile]);
  ...
  const getAllowedTabs = () => {
    ...
    let tabs = [
      ...
      { id: 'social', label: 'Social Media', icon: Instagram, req: 'social' },
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <Card className="p-6 border-t-4 border-t-purple-500">
          <h3 className="font-bold text-xl text-slate-800 dark:text-slate-200 mb-6 flex items-center gap-2">
            <Instagram className="w-6 h-6 text-purple-500" /> Meta Ads Management
          </h3>
  ...
  const renderSocialTab = () => {
    ...
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        {renderTable('reels', 'Video Reels Calendar', tReels, <Instagram className="w-5 h-5 text-rose-500" />)}
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className={`border rounded-xl p-4 flex flex-col items-center text-center transition-all ${rStatus.social ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
              <Instagram className={`w-6 h-6 mb-2 ${rStatus.social ? 'text-emerald-500' : 'text-slate-400'}`} />
  ...
  const renderAgencySettingsTab = () => {
    ...
    const handleCreateUser = () => {
      ...
      const userObj = { id: Date.now(), username: newUserUsername, password: newUserPassword, name: newUserName, role: newUserRole };
  ...
  const handleLoginSubmit = () => {
    ...
    if (foundUser) { setTeamMemberProfile(foundUser); setShowLogin(false); } 
      ...
      <div className={`min-h-screen flex items-center justify-center ${theme === 'dark' ? 'dark bg-slate-900 text-white' : 'bg-slate-50 text-slate-900'}`}>
        <Card className="w-full max-w-md p-8 shadow-xl border-t-8 border-t-indigo-600">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleLoginSubmit} disabled={!isUsersLoaded} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-lg">
